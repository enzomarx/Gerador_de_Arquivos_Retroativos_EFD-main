...
def generate_files():
    ...
    try:
        ...
        with open(r'C:\Users\marxe\Documents\MODELO_EFD_COM_VALORES.txt', 'r') as model_file:
        ...
        with zipfile.ZipFile(os.path.join(output_dir, 'EFD_Arquivos.zip'), 'w') as zipf:
            for index, row in df.iterrows():
                ...
                content = content.replace('VARIAVEL_05', str(row['bruto']))
                content = content.replace('VARIAVEL_06', str(row['brutocen']))
                content = content.replace('VARIAVEL_PIS', str(row['pis']))
                content = content.replace('VARIAVEL_07', str(row['piscen']))
                content = content.replace('VARIAVEL_COF', str(row['cof']))
                content = content.replace('VARIAVEL_08', str(row['cofcen']))
...
text_box = ctk.CTkTextbox(app, height=150, width=400)
